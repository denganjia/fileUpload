...
from models import Student, Teacher, Job, conn
...
class Work:
    ...
    def job_need_upload(schoolNo):
        ...
        for i in (res_l):
            jobs = Job.query.filter_by(job_id=str(i))  # "select job_name from jobs where job_id = '%s'" % i
            # cursor.execute(sql)
            # res = cursor.fetchall()
            for job in jobs:
